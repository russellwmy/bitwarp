    ...
    fn fmt(&self, fmt: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ErrorKind::DecodingError(e) => {
                write!(fmt, "Something went wrong with parsing the header. Reason: {:?}.", e)
            }
            ...
            ErrorKind::PacketError(e) => {
                write!(fmt, "Something went wrong with receiving/parsing packets. Reason: {:?}.", e)
            }
            ...
            ErrorKind::CouldNotReadHeader(header) => {
                write!(fmt, "Expected {} header but could not be read from buffer.", header)
            }
        }
    }
}
